...
local function open_notes_window(opts)
	...
	if notes_state.floating.win == -1 then
		notes_state.floating.win = Snacks.win.new(mergeAndReturnTable(window_config, {file = file_path}))
...
vim.api.nvim_create_user_command("NCreate", open_notes_window, {})
vim.keymap.set("n", "<A-n>", open_notes_window,{desc = "Open notes"})
